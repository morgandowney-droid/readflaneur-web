...
- **Daily brief CTAs:** `NeighborhoodBrief.tsx` - when expanded, shows two discovery links below source attribution: "Read the [Name] Daily Brief" (nearby mode) and "Take me somewhere new" (random mode, excludes current city). Lazy-fetched on first expand (two parallel API calls), cached in component state.
