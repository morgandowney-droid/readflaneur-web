...
- **Brief article cron** (`generate-brief-articles`): Already filters `.not('enriched_content', 'is', null)`, and no longer has a raw content fallback. Neighborhood+date dedup keeps only the latest brief per neighborhood per day (prevents duplicate articles from concurrent deployments). Uses actual `enrichment_model` from brief (was hardcoded to `gemini-2.5-flash`).
...
- **Source verification:** `SourceAttribution` shows source attribution for single-source articles. Props: `headline`, `neighborhoodName`, `editorNotes` passed from article page. Uses same dotted-underline academic link styling.
