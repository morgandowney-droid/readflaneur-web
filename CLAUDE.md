...
- **"Read Stories" button:** `HomepageEnterButton` (`src/components/home/HomepageEnterButton.tsx`) - client component, calls `/api/location/detect-and-match` (5s timeout), saves neighborhoods to localStorage, `router.push('/feed?welcome={city}')`. Graceful fallback to `/feed` on failure. Uses `btn-secondary` styling, `hero-fade-in-delay-4` (1.2s delay).
- **Returning users:** Inline `<script>` in `layout.tsx` reads localStorage and redirects to `/feed` before React hydration (no homepage flash, unchanged)
...
- **Cron:** `generate-look-ahead` (`0 17-22 * * *`, maxDuration=300, 270s budget) - runs hourly 5-10 PM UTC (6 runs), publishes for 7 AM local next morning. Single-pass: Grok search -> Gemini Flash enrichment -> article creation. Dedup skips already-processed neighborhoods across runs.
- **Priority:** Neighborhoods sorted by delivery urgency - those whose 7 AM local comes soonest (APAC/East) are processed first
- **Scope:** Only neighborhoods with active subscribers (via `getActiveNeighborhoodIds()` from `src/lib/active-neighborhoods.ts` - merges `user_neighborhood_preferences` + `newsletter_subscribers`)
- **Grok function:** `generateLookAhead()` in `grok.ts` - forward-looking search framed as "tomorrow morning" (since cron runs evening before). Explicit date context so Grok knows publication date.
- **Enrichment:** `articleType: 'look_ahead'` in `brief-enricher-gemini.ts` - no greeting/sign-off, organized by "Today" then "This Week", only verified events. `briefGeneratedAt` set to tomorrow 7 AM so Gemini CURRENT TIME context matches reader's perspective. Uses Flash exclusively (10K RPD, no Pro budget pressure).
...
- **API:** `GET /api/briefs/look-ahead?neighborhoodId=xxx` - returns most recent Look Ahead article URL (last 48h)
- **Feed card:** `LookAheadCard` (`src/components/feed/LookAheadCard.tsx`) - self-fetching, renders below daily brief. "LOOK AHEAD" eyebrow + "What's Coming Up in {name}" link. Wired into single-feed `page.tsx` and `MultiFeed.tsx`.
- **Discovery CTAs:** Look Ahead link added to `BriefDiscoveryFooter` (between yesterday and add-to-neighborhoods) and `NeighborhoodBrief` expanded view (between yesterday and nearby)
- **Email:** Look Ahead URL fetched in `assembler.ts` (`fetchLookAheadUrl()`), passed via `DailyBriefContent.lookAheadUrl`. Rendered in `DailyBriefTemplate` (after stories, before satellites) and `SundayEditionTemplate` (after THE NEXT FEW DAYS section). Link text: "Read the Look Ahead (next 7 days) for {name}".
...
### Image Library (Pre-Generated)
- **Core:** `src/lib/image-library.ts` - types, `selectLibraryImage()`, `getLibraryReadyIds()`, `checkLibraryStatus()`
- **Generator:** `src/lib/image-library-generator.ts` - Gemini Pro "creative director" prompts + Imagen 4 generation. Count tracking: full refreshes (no `skipExisting`) set `images_generated` to current run's count only; `skipExisting` mode adds to existing count. Prevents inflated counts when generation is interrupted by rate limits.
...
- **Selection:** `selectLibraryImage(neighborhoodId, articleType, categoryLabel?, libraryReadyIds?)` - returns library URL if neighborhood has generated images, `''` otherwise (retry-missing-images fills empty URLs). All 6 article-creating crons pre-fetch `libraryReadyIds` via `getLibraryReadyIds(supabase)` at startup and pass to each call.
- **Admin endpoint:** `POST /api/admin/generate-image-library` - batch generation with pagination (`batchSize`, `startFrom`, `neighborhoodId`, `forceRegenerate`). `GET` returns status counts.
- **Automated generation:** `refresh-image-library` cron (every 4h) generates ~3 neighborhoods per run (~9/day). Stops on daily quota exhaustion. Once all complete for current season, runs are instant no-ops. Tracks `generation_season` per neighborhood. Emails admin on completion.
- **Manual upload:** `scripts/upload-manual-images.mjs` - processes manually-generated images (e.g. from Gemini mobile app). Strips Gemini watermark via `sharp`, resizes to 1280x720, uploads to correct storage paths. Convention: name Sunday image with "sunday" in filename, other 7 are assigned to rotation categories. Numbered priority folders in `manual-images/` (e.g. `01-nyc-dumbo/`).
- **Status dashboard:** `scripts/image-library-status.mjs` - shows all neighborhoods sorted by subscriber count (priority), with complete/missing/partial status. Flags: `--missing` (only incomplete), `--search xyz` (filter by name/city/country).
- **Imagen 4 quota:** 70 RPD on Paid Tier 1 (shared across standard + fast models). 10 RPM. Resets midnight Pacific. Generator distinguishes daily quota exhaustion from per-minute rate limits - only retries on per-minute, bails immediately on daily exhaustion.
- **Cost:** ~$0.33/neighborhood ($0.01 Gemini Pro prompts + 8 x $0.04 Imagen 4). ~$89 for all 270 neighborhoods. Quarterly refresh ~$89.
- **DB:** `image_library_status` table tracks generation progress per neighborhood
- **Fallback:** `generate-image` endpoint tries library first (HEAD check), falls through to Gemini Image for community neighborhoods without library
- **Legacy:** `retry-missing-images` cron tries library first, falls back to Gemini Image
...
- **Sender:** `src/lib/email/sender.ts` — React Email via Resend. Sender name: "Flaneur News". Daily Brief subject: `Daily Brief: {Neighborhood}. {teaser}` (under 70 chars, 2-4 word phrases from story headlines joined with ", " and " & ").
- **Sunday Edition subject:** `Sunday Edition: {Neighborhood}. {teaser}` (under 70 chars, phrases from rearview stories + horizon events). `buildSundaySubject()` defined in both cron and on-demand routes.
- **Sunday Edition:** `src/lib/weekly-brief-service.ts` — Gemini Pro-first/Flash-fallback + Grok. Cron checks Pro RPD usage at start, uses Pro until budget exhausted (~5 calls/neighborhood), then falls back to Flash. Model param threaded through all 7 Gemini calls. Sections: The Letter, The Next Few Days, That Time of Year, Data Point, Your Other Editions. Cron processes 3 neighborhoods concurrently. Enrichment uses `weeklyRecapStyle` which forces Sunday framing regardless of processing day.
...
- **Category labels:** `cleanCategoryLabel()` in assembler.ts strips neighborhood prefix, then renames "Daily Brief" to "Daily Brief (Today)" and "Look Ahead" to "Look Ahead (next 7 days)" for email display. Appended with dateline: "Daily Brief (Today) - Wed Feb 18".
...
- **Current models:** Claude Sonnet 4.5, Gemini 2.5 Flash (enrichment fallback/translation), Gemini 2.5 Pro (enrichment primary + Sunday Edition + image library prompts, 1K RPD budget), Gemini 3 Pro Image (legacy image gen fallback), Imagen 4 (image library generation, $0.04/image), Imagen 4 Fast (community neighborhood library, $0.02/image), Grok 4.1 Fast
...
- **Source verification:** `SourceAttribution` shows "Single-source story - verify on Google" link when article has 0-1 sources. Google Search query = `headline + neighborhoodName`. Props: `headline`, `neighborhoodName` passed from article page. Uses same dotted-underline academic link styling.
