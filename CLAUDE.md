...
- **Scheduler:** `src/lib/email/partner-scheduler.ts` - resolves branded recipients from `client_emails` array + `newsletter_subscribers` with matching `partner_agent_id`. Integrated into `send-daily-brief` cron after standard sends.
- **Pricing:** $999/month per neighborhood, exclusive (one agent per neighborhood). Stripe subscription via `/api/partner/checkout`
...
- **API routes:** `POST /api/partner/setup` (upsert), `GET /api/partner/check-neighborhood` (availability), `POST /api/partner/preview` (send preview email), `POST /api/partner/checkout` (Stripe session), `GET /api/partner/status` (setup state), `POST /api/partner/upload` (photo upload), `POST /api/partner/subscribe` (client signup)
- **Stripe webhook:** Handles `checkout.session.completed` (activate), `customer.subscription.deleted` (cancel), `customer.subscription.updated` (pause on payment failure) via metadata `type: 'partner'`
- **Isolation:** Standard Flaneur subscribers never receive branded emails. Only clients via `/r/[slug]` or agent's `client_emails` list get branded version. A client who is both gets two emails (by design).
