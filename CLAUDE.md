...
- **Files:** `src/lib/cron-monitor/health-checks.ts` (check functions), `src/lib/cron-monitor/auto-fixer.ts` (auto-fix handlers), `src/lib/cron-monitor/health-report-email.ts` (email template), `src/app/api/cron/check-daily-health/route.ts` (cron endpoint), `src/app/api/cron/monitor-and-fix/route.ts` (auto-fix cron)
...
- **Unsplash client:** `src/lib/unsplash.ts` - API client with single-search strategy: ONE search per neighborhood (`"{name} {city}"`), take top 10 results, assign different photos to each of 8 categories by index. Tier 2 fallback search (`"{city} street neighborhood"`) if <8 results. Throws on rate limit (403/429) so crons can stop early. Triggers download endpoint for attribution tracking (required by Unsplash terms).
...
- **Rate limits:** Demo: 50/hr (backfill takes ~11 hours). Production: 5000/hr (all 273 in one run). Application submitted, pending 5-10 business day review.
...
- **Sender:** `src/lib/email/sender.ts` — React Email via Resend. Sender name always "Flaneur News" (extracts email from `EMAIL_FROM` env var, wraps with display name - env var contained `Flaneur <...>` which bypassed old regex). Daily Brief subject: `Daily Brief: {Neighborhood}. {lead headline} & more` (under 70 chars, uses full lead headline truncated at word boundary, appends "& more" if multiple stories). Preview text: "Your morning brief from {Neighborhood}+" (+ appended when satellite neighborhoods exist).
...
- **Weather:** Pure logic in `src/lib/email/weather-story.ts` (no LLM). Priority hierarchy: 1) Safety/Extremes (blizzard, extreme heat) → red-bordered WeatherStoryCard, 2) Travel & Lunch (weekday rain probability) → inline grey hint, 3) Weekend Lookahead (Thu/Fri only), 4) General Anomaly (forecast vs climate normals). Non-alert stories render as 13px #999999 text inside hero block (each sentence on own line). All probability displays use "% prob" suffix.
...
- **Grok-powered event crons (9 crons):** Shared `grokEventSearch()` utility in `grok.ts` (Responses API with `web_search` + `x_search`, temperature 0.5). Each cron's system prompt requests JSON array, caller parses with `raw.match(/\[[\s\S]*\]/)` then validates against existing configs (brand whitelists, venue lists, airline configs, Blue Chip keywords). Crons: overture-alert (9 opera/ballet venues), museum-watch (18 museums), sample-sale (luxury brand validation from 113 patterns), gala-watch (charity events per hub city), route-alert (airline routes per airport hub), residency-radar (luxury brand pop-ups by season), archive-hunter (vintage luxury per consignment store), nyc-auctions (Big Three per house in NYC), global-auctions (Big Three per house per hub — London/Paris/Hong Kong/LA/Geneva). All preserve existing Gemini story generation + article insertion untouched.
