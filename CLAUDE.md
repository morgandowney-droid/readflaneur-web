...
Recent work: Sunday Edition cost optimization (per-call model split in `weekly-brief-service.ts` - only `editorialSynthesis` / "The Letter" runs on Gemini Pro; the other 5 Gemini calls per neighborhood - `significanceFilter`, `huntEventsWithGemini`, `curateEvents`, `generateDataPoint`, holiday curation - now always use Flash with `thinkingConfig: { thinkingBudget: 0 }` since they are classification/curation/search-grounded extraction that Pro adds nothing to; `sync-weekly-brief` cron `GEMINI_CALLS_PER_NEIGHBORHOOD` 5 -> 1 so the shared 1K Pro RPD budget stretches ~6x further and every neighborhood's Letter now gets Pro instead of only the first ~20-160 before falling back to Flash; post-migration BigQuery billing showed the Sunday Edition was the readflaneur Gemini Pro-cost driver - a ~$27-30/day weekend spike vs ~$4-7 on weekdays, ~$245/mo - and this cuts it toward ~$80-110/mo while raising Pro-quality coverage), Broker campaign 2 launched (3-day sample-then-pitch sequence framed around the $999 -> $299 price drop and "your neighborhood is still available" scarcity; new `/api/cron/campaign-2` daily Vercel cron at 06:00 UTC pipelines all three touches with a 20-hour gap safeguard - touch 1 fires up to 40 fresh branded sample sends, touch 2 fires for brokers whose touch 1 is 20+ hours old, touch 3 fires the marketing pitch for brokers whose touch 2 is 20+ hours old; new `partner_sample_requests` and `c2_sample_1_sent_at`/`c2_sample_2_sent_at`/`c2_pitch_sent_at` columns on `broker_outreach`; manual `scripts/send-campaign-2.mjs` for staged dry-runs and limit control; `scripts/recheck-bounced-brokers.mjs` re-verifies the 75 originally-bounced addresses via Hunter.io and flips recovered ones back to pending - recovered 3 of 75 on first pass; sample sends use a new `isSampleCampaign` flag on `BrandedDailyBriefTemplate` that renders an amber "SAMPLE EDITION" banner above the masthead and routes from `sample@outreach.readflaneur.com` instead of the per-neighborhood mailbox so the recipient sees a clearly-labeled demo, body still shows the broker's real name + brokerage in the "Curated by" line so they can picture their own branding; full ~333-broker queue cycles to completion by ~May 18-20), Partner page lead-capture form (new `BrokerSampleRequest` component on `/partner` lets brokers enter their email + neighborhood and receive a real branded Daily Brief sample within ~60s; powers `/api/partner/sample-request` which is rate-limited 3/hr per IP and 1/24h per email, writes three rows on success - `partner_sample_requests` for rate-limit log + telemetry, `partner_waitlist` source='sample_request' for lead follow-up, and `newsletter_subscribers` for the flipped-funnel auto-subscribe so the broker also starts reading the consumer brief organically; "Use a neighborhood near me" button uses `navigator.geolocation` then falls back to IP via the now-coord-accepting `/api/location/detect-and-match`; form sits BELOW the hero on the page, not above - learned the hard way that cold visitors need value-prop framing before being asked for their email), Partner setup preview disclosure (collapsible `<details>` above the step circles on `/partner/setup` showing all 6 step labels + one-line descriptions so brokers see the full ask up front before starting - especially helps with the client-emails step which can spook brokers who don't know it's coming), Top nav promoted "Real Estate Partners" from footer to primary nav (between STORIES and the right-side icon cluster, translated in all 9 languages via new `nav.partners` key; search magnifying glass moved from the center nav to between ThemeToggle and LanguageToggle in the right cluster; homepage "Broker Dashboard ->" link renamed "Real Estate Agent Dashboard ->"), Partner page polish (pricing card widened max-w-md -> max-w-2xl so feature bullets do not wrap mid-sentence; FAQ "Can anyone read this content on your website?" relocated below "Could we build this ourselves?" and renamed "Can anyone read these news stories on the readflaneur website?"; "The product is running today." lead-in removed above the sample preview tiles; suggest-a-neighborhood form on `/destinations` capped at max-w-xl so it does not stretch the full header width), Supabase Data API grants change (Supabase emailed about Oct 30 2026 cutover where new tables in public schema no longer auto-expose to the Data API - documented the required `GRANT SELECT, INSERT, UPDATE, DELETE ON public.your_table TO service_role` pattern in CLAUDE.md "Critical Gotchas"; existing tables keep their current grants so no retroactive fix needed; audited all 26 known tables for service_role access, all clean), Partner pricing dropped $999 -> $299/mo (`PARTNER_PRICE_CENTS` 99900 -> 29900 in `api/partner/checkout/route.ts`, updated across `/partner` landing card, setup step 6, Stripe webhook copy, broker pitch scripts, and `outreach/broker-research-prompt.md` - $999 cold blast yielded zero conversions; lower price removes price-objection friction while keeping exclusive territory as the hook), Stripe webhook misrouting fix (deleted leftover `elegant-finesse` destination on the personal Morgandowney Stripe account `acct_1SlWGEEde7NIB89L` that was duplicating Oil101 events to readflaneur and tripping signature verification - had caused Stripe to disable the readflaneur webhook on May 1 after 9 days of consecutive failures; Oil101's correctly-routed `oil101-v2` destination was unaffected so no events actually missed; all Flaneur revenue still flows through Wooden Table Press LLC `acct_1TNxZbRoWuqglXpr`), Search performance + reliability (migration `20260509_articles_trigram_search_index.sql` enables `pg_trgm` and adds partial GIN trigram indexes on `articles.headline`/`body_text`/`preview_text` `WHERE status='published'` so `ILIKE '%query%'` uses the index instead of a seq scan; `src/app/search/page.tsx` wraps fetches in `AbortController` so stale responses cannot overwrite fresh ones - was producing intermittent "No results found" because Vercel timeouts on the slow ILIKE were silently rendered as empty results), Destinations admin gate + search across map bounds (server-side `profiles.role === 'admin'` check via SSR client gates the 33 Irish syndication-only entities `region='test'`; non-admins get empty `testDestinations` so the bottom-left "MD" toggle self-hides; map-bounds filter now only applies when search is empty so queries like `auk` correctly surface Montauk + Auckland-area neighborhoods even when map is zoomed elsewhere - `fitBoundsKey` effect re-fits map to search results), Broker cold-to-paid outreach infrastructure (new `broker_outreach` table with 10-state drip lifecycle; `/api/broker-drip/unsub` opt-out endpoint with token-based auth; `/api/cron/broker-drip` daily cron that syncs newsletter subscribers back to outreach state, auto-stops on neighborhood-taken, and sends touch 2 + touch 3 with volume cap; flipped-funnel strategy where touch 2 invites broker to free consumer subscription rather than re-pitching paid — addresses price objection + brand unknown failure modes; `scripts/send-drip-touch2-thursday.mjs` for curated 24-broker Thursday batch; `scripts/send-warmup.mjs` for Wednesday subdomain warming; backfilled 407 rows from Monday's cold blast CSVs). Dual-domain architecture: `readflaneur.com` for product mail (Daily Brief, Stripe, welcome), `outreach.readflaneur.com` for all cold outreach — separate Resend-verified subdomain with own SPF/DKIM/DMARC so cold-email reputation cannot contaminate the main domain's Daily Brief deliverability. Hunter.io email verification (`scripts/verify-emails-hunter.mjs`) catches invalid addresses before send. Em dashes banned in all user-facing content. Broker waitlist + nearby-suggestions when neighborhood is taken (new `partner_waitlist` table with `source='setup_blocked'|'cold_pitch'`; `/api/partner/check-neighborhood` auto-saves broker interest on upsert + returns top 5 Haversine-sorted nearby available neighborhoods within 15km; `pitch-preview` endpoint upserts cold-pitch recipients with source='cold_pitch'; `customer.subscription.deleted` webhook queries waitlist and emails each entry with a pre-filled setup URL stamping notified_at to prevent double-sends; setup page UI shows inline "Get notified if this opens up" email capture or waitlist confirmation + clickable nearby suggestions with "These nearby neighborhoods are still available - act fast" copy), Weather hint no longer word-wraps (tomorrowBox maxWidth 280→520px in both templates), Broker signup flow polish (8 mobile bugs found during end-to-end test) - scroll-to-top on step advance via `useEffect` on currentStep + clear stale error banner; copy-to-clipboard feedback ("Copied!" for 2s via `copied` state); subscribe-link label clarifies it's also emailed separately; new `sendSubscribeLinkEmail()` in `api/partner/setup/route.ts` fires fire-and-forget on partner creation so broker gets `/r/{slug}` link in inbox before activation; `handlePhotoUpload` rewritten to return boolean + 5MB+MIME client-side check + 30s AbortController timeout + actionable error messages (iPhone photos routinely 10-20MB were stalling silently on cellular); listing onChange uses functional setState `setEditingListing(prev => ({...prev, photo_url}))` to avoid stale-closure data loss; both photo inputs reset `e.target.value` after attempt; "US$299 - Billed in USD" on landing + step 6 (disambiguates for AU/CA/SG/HK brokers); `areaUnit` auto-derives `Sqft` for US vs `m²` elsewhere from `selectedNeighborhood.country`; subscribe-link input gets `min-w-0 text-xs` so long slugs don't bleed off mobile viewport; 14-day trial billing terms now explicit on both surfaces ("First billing starts 14 days after activation, then monthly on the 15th day after activation. You can cancel anytime before or after the free trial."), Simplify partner client offering (broker's clients now receive ONLY the branded Daily Brief, every day at 7 AM local time including Sundays - no Sunday Edition; `send-daily-brief` cron skips standard sends on Sunday but still runs the branded partner block; `resolveRecipients()` in scheduler.ts excludes `partner_agent_id IS NOT NULL` from both standard Daily Brief and Sunday Edition; updated `/partner` landing copy "What Your Clients Receive" to say "One email every day, delivered at 7 AM in the neighborhood's local timezone - 365 mornings a year" and removed the Sunday Edition block), Add broker discovery link to homepage hero (subtle "Real estate agents: partner with us →" link in muted uppercase below the "Read Stories" button, routes to /partner), Analytics instrumentation for email capture + onboarding + ad surfaces (client-side `track()` in `src/lib/analytics.ts` POSTs to `/api/analytics/track` which writes to `analytics_events` table with allowlisted event names; instrumented NewsletterSignup, EmailCaptureCard, /onboard page, AdCard, StoryOpenAd, FallbackAd bonus/house/static blocks; events: newsletter_signup.view/submit/success/error, email_capture.view/submit/success/error/dismiss, onboard.step1.view/continue, onboard.step2.view/google_click/submit/success/error, ad.impression, ad.click), Unify saved neighborhoods (removed destination lists system - deleted useDestinationLists hook, AddToListModal, /api/lists/ routes, /lists/[shareToken] page, ~1,575 lines removed, 10 files deleted - heart icon badge and WishlistDropdown now use flaneur-neighborhood-preferences localStorage, one concept: save = subscribe to neighborhood feed), Remove auto-redirect for returning users (all visitors see cinematic homepage at readflaneur.com, "Read Stories" routes to /feed if neighborhoods saved or /onboard if new, cookie sync preserved), Agent partner system for luxury real estate agents (self-serve /partner setup page with 6-step flow, /r/[agent-slug] subscribe links, BrandedDailyBriefTemplate with agent header/listing cards/branded footer, sendBrandedDailyBrief() sender with "James Chen: Tribeca Daily" from line, partner-scheduler for branded sends integrated into daily brief cron, Stripe $299/month subscription with webhook handling, agent_partners DB table with one-agent-per-neighborhood constraint and partner-assets storage bucket), Simplify destinations page (removed complex LC-style filter system - ALL FILTERS slide-out panel, COASTAL/SLOPES/COLLECTIONS dropdowns, region/country/neighborhood-type filters, 9 state variables, ~441 net lines removed - replaced with clean search bar + 3-option sort dropdown nearest/A-Z/region, map and Table View and cards untouched), Fuzzy search for neighborhoods (Levenshtein edit distance matching in resolveSearchQuery so typos like "aukland" find "Auckland", 1 edit for 3-5 char queries, 2 for 6+, search API uses resolveSearchQuery server-side for fuzzy neighborhood matching), Search page searches neighborhoods alongside articles (dedicated section above article results with location pin icons, search.neighborhoods translation key in all 9 languages), Look Ahead generation for ALL active neighborhoods not just those with subscribers (schedule hourly 0-7 UTC), BriefDiscoveryFooter shows Look Ahead link + 2 nearest neighborhood daily brief links, Rename "AI Standards & Ethics" to "Standards & Ethics" across all 9 languages
...
- **Sunday Edition:** `src/lib/weekly-brief-service.ts` — Gemini + Grok. **Per-call model split (2026-05-15):** only `editorialSynthesis` ("The Letter") uses Gemini Pro; the other 5 Gemini calls per neighborhood (`significanceFilter`, `huntEventsWithGemini`, `curateEvents`, `generateDataPoint`, holiday curation) always use Flash with `thinkingConfig: { thinkingBudget: 0 }` — they are classification/curation/search-grounded extraction where Pro adds nothing. So each neighborhood consumes 1 Pro call, not ~5. `sync-weekly-brief` cron still checks Pro RPD usage at start (`GEMINI_CALLS_PER_NEIGHBORHOOD = 1`) and falls the Letter back to Flash only if the shared 1K Pro RPD budget is exhausted — but at 1 call/neighborhood the budget now covers all 270 Letters. Sections: The Letter, The Next Few Days, That Time of Year, Data Point, Your Other Editions. Cron processes 5 neighborhoods concurrently (~10/run at 280s budget). Schedule: `0 * * * 6,0` (hourly Saturday + Sunday, 48h window for full 270 neighborhood coverage). `weekDate` always calculates "this coming Sunday" (if already Sunday, use today; otherwise add days until Sunday) so Saturday and Sunday runs share the same date for dedup. Enrichment uses `weeklyRecapStyle` which forces Sunday framing regardless of processing day. Banned generic opener "quiet significance" and variants.
...
- **Pricing:** $299/month flat per neighborhood, exclusive (one agent per neighborhood). Dropped from $999 (2026-05-09) after the $999 cold blast yielded zero conversions; lower price removes price-objection friction while keeping exclusive territory as the scarcity hook. **14-day free trial** via Stripe `subscription_data.trial_period_days: 14` with `missing_payment_method: 'cancel'` fallback and `payment_method_collection: 'always'` so card is captured up front. Stripe subscription via `/api/partner/checkout`.
...
### Broker Outreach & Drip (Cold-to-Paid Funnel)
- **Dual-domain architecture** to protect product deliverability. `readflaneur.com` handles all product email (Daily Brief, Stripe receipts, broker welcome, branded partner sends). `outreach.readflaneur.com` is a separate Resend-verified subdomain used exclusively for cold outreach and drip follow-ups, with its own SPF/DKIM/DMARC. Replies from outreach emails route back to `md@readflaneur.com` via the `reply-to` header. This prevents cold-email reputation from contaminating the main domain used by real subscribers.
- **Table:** `broker_outreach` (migration `20260421_broker_outreach.sql`) - one row per (broker_email, neighborhood_id) pitched. 10-state machine: `pending` → `active` (touch 2 sent) → `subscribed` → `warm` → `converted`; plus terminal states `unsubscribed`, `bounced`, `neighborhood_taken`, `ooo`, `not_interested`. Each row has an `unsub_token` (random 32-char hex) used in the unsub URL.
- **Flipped-funnel strategy:** touch 1 pitches the paid product directly (the `send-broker-pitches.mjs` cold blast). Touch 2 pivots to inviting the broker to subscribe to the FREE consumer newsletter for their neighborhood, so they can judge the editorial quality for 2-3 weeks before being pitched paid. Touch 3 fires only after they've been a subscriber for 14+ days, pitching the branded paid version to a warm audience. This addresses the two diagnosed failure modes of touch 1 (price objection from unknown sender, zero brand recognition).
- **Cron:** `/api/cron/broker-drip` (not yet in vercel.json — manual trigger with `CRON_SECRET` during rollout). Does four things per run: (1) syncs `newsletter_subscribers` → `broker_outreach` to flip rows to `subscribed` status when a broker opts into the consumer product, (2) auto-stops brokers whose neighborhood now has an active `agent_partners` row, (3) sends touch 2 to `pending` rows with `touch_1_sent_at` >= 2 days ago, (4) sends touch 3 to `subscribed` rows with `subscribed_to_newsletter_at` >= 14 days ago. Daily volume cap: 100 emails to protect outreach subdomain warmup.
- **Unsub endpoint:** `GET /api/broker-drip/unsub?token=xxx` - no auth, token is proof of identity. One click flips ALL rows for that broker_email across all neighborhoods to `unsubscribed`. Returns a friendly HTML page. Also exposed via `List-Unsubscribe` header on every drip email for Gmail/Outlook native one-click unsub.
- **Thursday one-shot:** `scripts/send-drip-touch2-thursday.mjs` - manual trigger for a hand-curated batch of 24 top brokers (NYC Tribeca + UES + West Village + London Mayfair + Belgravia + Notting Hill + Stockholm Östermalm). Runs the flipped-funnel touch 2 email. Dry-run mode default; `--fire` flag to actually send. Used to validate the approach before the cron takes over Friday onward.
- **Warmup:** `scripts/send-warmup.mjs` - Wednesday manual trigger to send 2 internal test emails from `md@outreach.readflaneur.com` to Morgan's Gmail during the 48h pause, so Gmail sees continued positive activity on the subdomain before Thursday's 24-broker batch.
- **Subscriber attribution:** rather than threading `?broker_ref=xxx` URL params through the consumer newsletter signup (fragile, breaks on share/forward), the drip cron does a post-hoc email match: queries `newsletter_subscribers` by email for every `pending`/`active` broker_outreach row and promotes matches to `subscribed`. Robust to all subscription paths.
- **Touch-1 send script:** `scripts/send-broker-pitches.mjs` - unchanged behavior but `from` field now reads `Morgan Downey <md@outreach.readflaneur.com>`. Reply-to stays `md@readflaneur.com`. Pitch copy uses localized paper references per neighborhood_id prefix (FT HTSI for London, Le Figaro for Paris, etc.). No em dashes anywhere (pet peeve).
- **Deliverability learnings (2026-04-20 onward):** Original Monday blast from `md@readflaneur.com` had 18.5% hard bounce rate due to pattern-guessed emails for walled firms (Knight Frank, Savills UK). Dented main domain reputation. Solution: (a) separated to `outreach.readflaneur.com`, (b) integrated Hunter.io verification (`scripts/verify-emails-hunter.mjs`, `HUNTER_API_KEY` env var) to pre-check addresses, (c) gradual volume curve on new subdomain (day 1: 2 emails, day 2: 5, day 3: 24, day 4-7: 40-80/day, day 8+: full). Brand new subdomain's first send landed in spam — normal and expected. After user marked "not spam" + added to contacts, second realistic-content test landed in Primary.
