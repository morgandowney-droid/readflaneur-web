...
- **Model:** `gemini-2.5-flash` (switched from Pro due to 1K RPD limit; Flash has 10K RPD)
...
- **Sunday Edition:** `src/lib/weekly-brief-service.ts` — Gemini + Grok. Sections: The Letter, The Next Few Days, That Time of Year, Data Point, Your Other Editions. Cron processes 3 neighborhoods concurrently; within each, Horizon/DataPoint/Holiday sections run in parallel via `Promise.allSettled`. Enrichment uses `weeklyRecapStyle` which forces Sunday framing regardless of processing day.
...
- **Current models:** Claude Sonnet 4.5, Gemini 2.5 Flash (enrichment/translation), Gemini 2.5 Pro (Sunday Edition only - 1K RPD limit), Gemini 3 Pro Image (image gen), Grok 4.1 Fast
