...
- **Invite page:** `/invite` - dual-mode page. **Inviter mode** (subscribed user, no `?ref=` param): "Share Flaneur" heading, personalized invite link in readonly input, "Copy Link" + "Share" buttons (native share on mobile), referral stats (clicks/friends joined) if > 0. **Invitee mode** (new visitor or `?ref=` param): streamlined hero with email input, auto-detects location on submit, subscribes, redirects to `/feed?welcome={city}`. No `SmartRedirect` (visitors must see the page). Detection: `localStorage.getItem('flaneur-newsletter-subscribed') === 'true' && !searchParams.get('ref')`.
