...
- **Cron:** `translate-content` (*/30, maxDuration=300, 250s budget). Translates articles/briefs from last 48h. Concurrency 3, exponential backoff on 429. **Language rotation:** Rotates start language each run (based on half-hour offset) so all 8 languages get fair coverage instead of always starting with sv. Logs to `cron_executions`.
...
- **Invite page:** `/invite` - streamlined hero with email input only. No neighborhood selector. On submit: auto-detects location via `/api/location/detect-and-match`, saves 4 nearest neighborhoods, subscribes, redirects to `/feed?welcome={city}`. No `SmartRedirect` (visitors must see the page).
...
- **Links:** Academic "invisible link" style - `text-current font-semibold underline decoration-dotted decoration-neutral-500/40 decoration-1 underline-offset-4`, hover: `decoration-solid decoration-neutral-300/60`. Applied to pipeline-injected links only (existing articles with markdown links in DB). **Auto-linking disabled** - render-time entity detection and pipeline hyperlink injection both removed to avoid cluttered articles. No amber/blue link colors anywhere.
