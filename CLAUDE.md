...
- **UI strings:** `src/lib/translations.ts` - ~269 keys per language, `t(key, language)` lookup with English fallback. `useTranslation()` hook wraps context + t().
...
- **NOT translated:** Email templates (stay English), admin pages, neighborhood/city names (proper nouns), ad content (advertiser-supplied), advertise booking page
