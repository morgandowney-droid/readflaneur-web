...
- **Cron:** `generate-look-ahead` (`0 20 * * *`, maxDuration=300, 270s budget) - runs 8 PM UTC, publishes for 7 AM local next morning. Single-pass: Grok search -> Gemini Flash enrichment -> article creation
...
- **Article:** `article_type: 'look_ahead'`, `category_label: '{Name} Look Ahead'`, deterministic slug `{id}-look-ahead-{tomorrowDate}-{headline}`, `published_at` set to tomorrow 7 AM UTC, concurrency 3
...
- **Email:** Look Ahead URL fetched in `assembler.ts` (`fetchLookAheadUrl()`), passed via `DailyBriefContent.lookAheadUrl`. Rendered in `DailyBriefTemplate` (after stories, before satellites) and `SundayEditionTemplate` (after THE NEXT FEW DAYS section).
- **Article pages:** `BriefDiscoveryFooter` rendered on `look_ahead` article pages (same as daily brief articles)
...
- **Core:** `src/lib/image-library.ts` - types, `selectLibraryImage()`, `checkLibraryStatus()`
- **Generator:** `src/lib/image-library-generator.ts` - Gemini Pro "creative director" prompts + Imagen 4 generation
...
- **Selection:** `selectLibraryImage(neighborhoodId, articleType, categoryLabel?)` - pure function, no DB/network calls. Used at article insert time in all 6 article-creating crons.
