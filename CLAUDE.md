...
- **Cron:** `generate-look-ahead` (`0 8 * * *`, maxDuration=300, 270s budget) - single-pass: Grok search -> Gemini Flash enrichment -> article creation
...
- **Grok function:** `generateLookAhead()` in `grok.ts` - forward-looking search for events, openings, exhibitions, community meetings over next 7 days
- **Enrichment:** `articleType: 'look_ahead'` in `brief-enricher-gemini.ts` - no greeting/sign-off, organized by "Tomorrow" then "This Week", only verified events. Uses Flash exclusively (10K RPD, no Pro budget pressure).
- **Article:** `article_type: 'look_ahead'`, `category_label: '{Name} Look Ahead'`, deterministic slug `{id}-look-ahead-{YYYY-MM-DD}-{headline}`, concurrency 3
- **Dedup:** Checks for existing `look_ahead` articles for today's date per neighborhood
