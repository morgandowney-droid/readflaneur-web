...
- **Scope:** Only neighborhoods with active subscribers (via `getActiveNeighborhoodIds()` from `src/lib/active-neighborhoods.ts` - merges `user_neighborhood_preferences` + `newsletter_subscribers`, expands combo IDs to component IDs). No `is_active` filter on neighborhoods query since combo components may be `is_active=false` but still need articles.
...
- **Tourist trap filter:** Both `grok.ts` (look-ahead system prompt) and `brief-enricher-gemini.ts` (`lookAheadStyle`) exclude long-running tourist shows (Mamma Mia, Lion King, Phantom of the Opera, Wicked, etc.) unless something unusual is happening (closing, reopening, major cast change, anniversary milestone).
...
- **Source of truth:** `flaneur-neighborhood-preferences` localStorage (array of IDs). Cookie is a sync copy for server-side reading.
...
- **Unsplash client:** `src/lib/unsplash.ts` - API client with 4-tier fallback search in `searchAllCategories()`: (1) exact `"{name} {city}"`, (2) reversed `"{city} {name}"`, (3) city-only `"{city}"`, (4) city+country `"{city} {country}"`. Accepts optional `country` param. Stops as soon as 8 photos found. Top 10 results assigned to 8 categories by index. Throws on rate limit (403/429) so crons can stop early. Triggers download endpoint for attribution tracking (required by Unsplash terms).
...
- **Bare /feed redirect:** Both links go to bare `/feed` (no query params). Feed reads neighborhood IDs from `flaneur-neighborhoods` SameSite=Strict cookie (synced from localStorage by inline script in `layout.tsx`). `MultiFeed` detects empty `neighborhoods` prop, reads localStorage, syncs cookie, and calls `router.refresh()`. Covers any path to bare `/feed` (back links, bookmarks, direct URL).
