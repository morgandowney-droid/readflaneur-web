...
- **Enrichment:** `articleType: 'look_ahead'` in `brief-enricher-gemini.ts` - no greeting/sign-off, organized by "Today" then "This Week", only verified events. Date headers include day of week (e.g., "Today, Wednesday February 18"). `briefGeneratedAt` set to tomorrow 7 AM so Gemini CURRENT TIME context matches reader's perspective. Uses Flash exclusively (10K RPD, no Pro budget pressure).
