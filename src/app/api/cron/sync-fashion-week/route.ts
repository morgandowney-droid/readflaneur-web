...
/**
 * @swagger
 * /api/cron/sync-fashion-week:
 *   get:
 *     summary: Sync fashion week calendar events
 *     tags: [Cron]
 *     security:
 *       - cronSecret: []
 *     responses:
 *       200:
 *         description: Fashion week sync completed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
  ...
  try {
    ...
    for (const story of stories) {
      try {
        ...
        // Create article with appropriate priority and cached image
        const { error: insertError } = await supabase.from('articles').insert({
          neighborhood_id: finalNeighborhoodId,
          headline: story.headline,
          body_text: story.body,
          preview_text: story.previewText,
          image_url: await getCronImage('fashion-week', supabase, { neighborhoodId: finalNeighborhoodId }),
          slug,
          status: 'published',
