...
/**
 * @swagger
 * /api/cron/sync-art-fairs:
 *   get:
 *     summary: Sync art fair calendar events
 *     tags: [Cron]
 *     security:
 *       - cronSecret: []
 *     responses:
 *       200:
 *         description: Art fairs sync completed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
        ...
        for (const { neighborhoodId, slug } of targetDistributions) {
          try {
            ...
            const { error: insertError } = await supabase.from('articles').insert({
              ...
              image_url: await getCronImage('art-fair', supabase, { neighborhoodId: finalNeighborhoodId }),
              slug,
              status: 'published',
