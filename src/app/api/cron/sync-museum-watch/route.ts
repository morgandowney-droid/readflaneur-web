...
/**
 * @swagger
 * /api/cron/sync-museum-watch:
 *   get:
 *     summary: Sync museum exhibitions and events
 *     tags: [Cron]
 *     security:
 *       - cronSecret: []
 *     responses:
 *       200:
 *         description: Museum watch sync completed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
      ...
      for (const neighborhoodId of story.targetNeighborhoods) {
        try {
          ...
          const { error: insertError } = await supabase.from('articles').insert({
            ...
            image_url: await getCronImage('museum-watch', supabase, { neighborhoodId: finalNeighborhoodId }),
            slug,
            status: 'published',
