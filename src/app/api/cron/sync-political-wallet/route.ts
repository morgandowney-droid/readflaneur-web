...
/**
 * @swagger
 * /api/cron/sync-political-wallet:
 *   get:
 *     summary: Sync political donations and lobbying data via FEC API
 *     tags: [Cron]
 *     security:
 *       - cronSecret: []
 *     responses:
 *       200:
 *         description: Political wallet sync completed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
      ...
      for (const neighborhoodId of story.targetNeighborhoods) {
        try {
          ...
          const { error: insertError } = await supabase.from('articles').insert({
            ...
            image_url: await getCronImage('political-wallet', supabase, { neighborhoodId: finalNeighborhoodId }),
            slug,
            status: 'published',
