  ...
  return (
    <div className="min-h-screen bg-canvas py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-light text-fg mb-2">Community Neighborhoods</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {[
            ...
            { label: 'Removed', count: counts['removed'] || 0, color: 'text-fg-muted' },
          ].map(stat => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-elevated">
                  <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-fg-muted font-medium">Name</th>
                  <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-fg-muted font-medium">City</th>
                  <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-fg-muted font-medium">Country</th>
                  <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-fg-muted font-medium">Creator</th>
                  <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-fg-muted font-medium">Status</th>
                  <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-fg-muted font-medium">Created</th>
                ...
                {filtered.map(n => (
                  <tr key={n.id} className="border-b border-white/[0.06] hover:bg-white/[0.02]">
                    <td className="px-4 py-3">
                      <p className="text-fg font-medium">{n.name}</p>
                      <p className="text-xs text-fg-subtle font-mono">{n.id}</p>
                    </td>
                    <td className="px-4 py-3 text-fg-muted">{n.city}</td>
                    <td className="px-4 py-3 text-fg-muted">{n.country || '-'}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-block px-2 py-1 text-xs tracking-widest uppercase ${
                        n.community_status === 'active'
                          ...
                          : 'bg-neutral-500/15 text-fg-muted'
                      }`}>
                        {n.community_status}
                      </span>
                    </td>
