...
**Source rows leaked placeholders and dead Gemini redirect URLs (brief + Look Ahead sources):** Found when Michael Fabiano (ex-AP Head of Americas Media Business) looked at yous.news county pages before replying to a pitch and noted stories with no link to a larger story; the yous.news side then rendered a Sources block from the Flaneur syndication feed and found Kilkenny with four plain names plus a "User provided content" entry, and Look Ahead links that 404. Three faults on the Flaneur side. (1) Placeholder names stored as publications: Gemini labels a story that came from the supplied Grok facts as "User provided content", and our own fallbacks in , , the assembler fallback and the auto-fixer inserted "X (Twitter)" / "Google News" platform rows when nothing better existed. (2) Google-Search grounding gives source URLs on ; they 302 to the real page for roughly two weeks then 404 (12,935 such rows in ). (3) Gemini often names a source with no URL even though the same response carries  (uri + title + domain) for the pages it read; the enricher never looked at them. In the last 36h of Irish content: 99 articles, 541 source rows, 87 null URLs, 25 redirects, 28 placeholders, 31 articles with no usable link. Fix: new  with , ,  (HEAD + Location, cached, never throws, ~450ms),  / , a conservative  (domain-root or full-word-coverage match, score >= 2, otherwise no guess),  applied inside  before anything is stored, and  which replaces the four copied extractors and never emits placeholders (an article with nothing checkable gets no rows; the article page already shows the generic line).  is now . The syndication route filters placeholders and nulls redirect URLs at the boundary for rows written earlier.  (dry run default, , ) cleans existing rows; run on Irish counties, last 3 days: 61 placeholders deleted, 41 redirects resolved, 5 dead nulled; name-only rows cannot be fixed retroactively. Rule: a source row must be a checkable publication, site or account with a URL that will still work in a month; never store a placeholder or a grounding redirect.
## 2026-09-03
