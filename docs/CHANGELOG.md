...
## 2026-09-26: an editor desk where GEDI's editors approve what their feed carries
...
**Not yet true.** Until the migration runs, the page reads everything but every action returns "cannot be saved yet" and says so on the page, and the GEDI feed is empty. The translated headline carries the section label ("IL PUNTO DEL GIORNO: ...") because that is what the feed returns today. There is no per-editor login: the key is shared by the desk and the name is self-declared.
